<?
	...
	while ($row=mysql_fetch_array($res)) {
		...
		$description = "<h2><a href=\"index.php?view=info&show=1&id=".$row['uid']."&cid=".$row['id']."\">".htmlspecialchars($row['name'], ENT_QUOTES)." - ".$row['uid']."</a></h2><table><tr><td><img style=\"max-width: 100px;\" src=\"/images/models/".str_replace('"', '', $row['model']).".png\"></td><td>&nbsp;</td><td style=\"vertical-align:top; \"><h2>Position:</h2>left:".round(($y/100))." top:".round(((15360-$x)/100))."</td></tr></table>";
		$markers .= "['".htmlspecialchars($row['name'], ENT_QUOTES)."', '".$description."',".$y.", ".($x+1024).", ".$k++.", '/images/icons/player".($row['death'] ? "_dead" : "").".png'],";
